...
export default function Dashboard() {
        ...
        <div className="max-w-5xl mx-auto px-4 py-8 animate-pulse">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
            {Array.from({ length: 4 }).map((_, idx) => (
              <div
                key={idx}
                className="bg-dark-800 rounded-xl border border-slate-700 p-5 flex items-center gap-4"
              >
            ...
            <div>
              ...
              <div className="space-y-2">
                {Array.from({ length: 4 }).map((_, idx) => (
                  <div
                    key={idx}
                    className="bg-dark-800 rounded-xl border border-slate-700 px-4 py-3 flex items-center justify-between"
                  >
                    <div className="space-y-2">
            ...
            <div>
              ...
              <div className="space-y-2">
                {Array.from({ length: 4 }).map((_, idx) => (
                  <div
                    key={idx}
                    className="bg-dark-800 rounded-xl border border-slate-700 px-4 py-3"
                  >
  ...
  const totalTopicsPages = Math.max(
    1,
    Math.ceil(topicStats.length / topicsPerPage),
  );
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col h-full">
            <h2 className="text-lg font-semibold text-white mb-3 flex items-center gap-2">
              <BarChart2 size={17} className="text-slate-400" /> Topics
              Practiced
            </h2>
            {topicStats.length === 0 ? (
              <div className="bg-dark-800 rounded-xl border border-slate-700 p-8 text-center">
              ...
              <div className="flex flex-col flex-1">
                  ...
                  {paginatedTopics.map((t) => (
                    ...
                    >
                      ...
                      <p className="text-slate-500 text-xs mt-1">
                        {t.sessions_count} session
                        {t.sessions_count !== 1 ? "s" : ""} ·{" "}
                ...
                {totalTopicsPages > 1 && (
                  <div className="mt-auto pt-3 flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        setTopicsPage((p) => Math.min(totalTopicsPages, p + 1))
                      }
